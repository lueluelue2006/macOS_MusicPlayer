...
MusicPlayer 是一个用 SwiftUI 和 AVFoundation 写的 macOS 本地音乐播放器。它不做云同步，不上传音乐库，重点是把本地文件播放、歌单、歌词、响度分析和随机权重这些日常功能做得直接、稳定。
...
搜索框支持按歌曲名、艺术家、专辑和文件名过滤。列表里的“均”标记表示该曲目是否已经完成音量均衡分析。

## 随机权重

随机权重只影响随机/洗牌/骰子，不影响顺序播放。

默认是绿色 `1.0x`，不会写入权重缓存。白色是更低权重，用来让歌更少出现；蓝色到红色会逐步提高出现概率。
