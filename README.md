...
从 **v3.5** 起，更新会把旧版本移动到废纸篓（`~/.Trash`），不会再在 `/Applications` 里堆积备份。若你之前已产生备份目录，可手动删除这些 `MusicPlayer.app.bak*` 文件夹。

提示：如果你是从 **v3.4 自动更新到 v3.5**，这一次仍可能留下一个备份目录（由旧版本更新脚本创建）；升级到 v3.5 后，后续更新不会再留下。
