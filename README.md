...
- 拖拽添加音乐文件或文件夹
- 队列播放和自定义歌单
- 杨坤、费玉清、孙燕姿、王菲歌单内置歌手封面，其他歌单可选择自定义图片
- 随机播放与单曲循环二选一
...
歌单页会为杨坤、费玉清、孙燕姿和王菲显示来自可再分发真人照片的内置歌手封面。点击歌单标题旁的封面即可选择自己的图片，右键封面或使用歌单菜单可以恢复默认；其他歌单没有图片时会继续显示轻量的字母封面。导入图片会先缩小到最长边 1,024 像素，界面只按实际显示尺寸解码，并把内存缓存限制在 16 张、约 8 MiB。内置照片的来源、作者、修改说明及独立许可见 [封面署名文件](assets/playlist-covers/ATTRIBUTIONS.txt)。

播放控制栏左侧是随机播放与单曲循环的滑动 Toggle，点按一次就会滑到另一侧，始终保留一种播放方式。搜索框支持按歌曲名、艺术家、专辑和文件名过滤。歌曲行标题旁的灰色对勾表示该曲目已经完成音量均衡分析；每首歌曲都在时长旁常驻显示六个随机权重方块，编辑和移除操作仍在悬停时出现。同步歌词工具栏中，红色表示自动跟随已开启，灰色表示已关闭，“当前句”保持为中性的定位操作。
...
缓存只保存响度结果、文件大小、修改时间和文件标识，不保存解码后的 PCM。结果写入 `Caches` 下的有界 SQLite 数据库，内存只保留最近使用的一小批记录；同一路径文件被替换后会自动失效，连续更新会合并写盘。这样可以用少量可重建缓存换取后续播放零分析开销，又不会让内存、CPU 或 JSON 无限增长。

## 队列恢复

队列、歌单、随机权重、外置磁盘位置和播放会话统一保存在 `Library.sqlite`。结构变更使用事务和单调修订号，连续操作会合并为最新提交；单纯切歌只更新队列游标，不重写整份曲库。退出时所有关键存储共享一个有界刷新期限，损坏或未来版本的数据库会进入保护界面，不会静默覆盖原数据。

## 随机权重

随机权重只影响随机播放、洗牌和骰子选择；切到单曲循环时，它不会影响当前歌曲的重复播放。

权重共有六档；界面按第 1 到第 6 档显示，CLI/API 的内部编号仍为 `0` 到 `5`。每首未单独设置的歌曲默认使用第 2 档（`1.0×`），不会写入权重缓存。队列和歌单的每首歌曲都常驻显示六个方块，也可以通过右键菜单或正在播放区域选择更低或更高的出现概率。
...
其中 `Library.sqlite` 是队列、歌单、随机权重、外置磁盘书签和播放会话的唯一权威；用户选择的封面等资源也保留在这里。元数据、时长、响度和沉浸播放边界属于可重建数据，放在：

```text
~/Library/Caches/MusicPlayer
```

偏好设置使用应用自己的 macOS preferences domain。旧版 JSON 和散落偏好会在首次升级时经过校验后自动迁移；迁移失败或遇到更高版本格式时，播放器保持只读保护并保留诊断数据。
