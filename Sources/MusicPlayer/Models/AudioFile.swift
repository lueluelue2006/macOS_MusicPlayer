    ...
    static func load(from metadataItems: [AVMetadataItem], asset: AVAsset?, includeArtwork: Bool = true) async -> AudioMetadata {
        ...
        if let urlAsset = asset as? AVURLAsset {
            if let v1 = ID3v1Reader.read(from: urlAsset.url) {
                func looksCorrupted(_ s: String) -> Bool {
                    if s.isEmpty { return true }
                    if s.contains("\u{FFFD}") { return true } // replacement character
                ...
                func usable(_ s: String?) -> String? {
                    guard let s, !looksCorrupted(s) else { return nil }
                    return s
                }
                // 条件1：缺失 -> 直接回补
                if title == "未知标题", let t = usable(v1.title) { title = t }
                if artist == "未知艺术家", let a = usable(v1.artist) { artist = a }
                if album == "未知专辑", let al = usable(v1.album) { album = al }
                if year == nil, let y = v1.year { year = y }
                // 条件2：疑似乱码（无中文）且 v1 含中文 -> 以 v1 覆盖
                if let t = usable(v1.title), ID3v1Reader.containsCJK(t), !ID3v1Reader.containsCJK(title) { title = t }
                if let a = usable(v1.artist), ID3v1Reader.containsCJK(a), !ID3v1Reader.containsCJK(artist) { artist = a }
                if let al = usable(v1.album), ID3v1Reader.containsCJK(al), !ID3v1Reader.containsCJK(album) { album = al }
            }
        }
