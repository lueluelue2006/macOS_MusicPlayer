...
actor MetadataCache {
    ...
    private func loadIfNeeded() {
        ...
        guard let data = try? Data(contentsOf: url) else {
            ...
            isReadOnly = true
            PersistenceLogger.log("元数据缓存文件存在但无法读取，进入只读模式保护数据")
            return
        }
        ...
        if let probe = try? JSONDecoder().decode(CacheVersionProbe.self, from: data),
           ...
           version > formatVersion {
            isReadOnly = true
            PersistenceLogger.log("检测到未来元数据缓存版本 \(version)（当前支持 \(formatVersion)），进入只读模式保护数据")
        ...
        guard let decoded = try? JSONDecoder().decode(CacheFile.self, from: data),
            ...
            isReadOnly = true
            PersistenceLogger.log("元数据缓存文件格式未知或损坏，进入只读模式保护数据")
            return
        }
